...
if ${ENABLE_OCCLUDED}; then
    for OCC_LEVEL in "${OCC_LEVELS[@]}"; do
        echo "Create OccludedPascal3D+ dataset L$OCC_LEVEL"
        python3 ./code/dataset/CreatePascal3DNeMo.py --overwrite False \
...
for MESH_D in "${MESH_DIMENSIONS[@]}"; do
    python3 ./tools/CreateCuboidMesh.py --CAD_path "${PATH_PASCAL3DP}" --mesh_d "${MESH_D}"
done

####################################################################################################
# Create 3D annotations
for MESH_D in "${MESH_DIMENSIONS[@]}"; do
    python3 ./code/dataset/generate_3Dpascal3D.py --overwrite False \
...
if ${ENABLE_OCCLUDED}; then
    for MESH_D in "${MESH_DIMENSIONS[@]}"; do
        for OCC_LEVEL in "${OCC_LEVELS[@]}"; do
            python3 ./code/dataset/link_annotations.py --source_path "${PATH_CACHE_TESTING_SET}" --target_path "${PATH_CACHE_TESTING_SET_OCC}" \
                    --occ_level "${OCC_LEVEL}" --mesh_d "${MESH_D}"
            python3 ./code/dataset/refine_list.py --root_path "${PATH_CACHE_TESTING_SET_OCC}" \
                    --occ_level "${OCC_LEVEL}" --mesh_d "${MESH_D}"
        done
    done
fi
